...
select count(distinct Row_id) from retail_sales;
...
count(*) as duplicate_count from retail_sales
group by Order_id,Order_date,Ship_date,Ship_mode,Customer_id,Customer_name,
Segment,Country,City,State,Postal_code,Region,Product_id,
Category,Sub_category,Product_name,Sales,Quantity,Discount,Profit
having count(*)>1;

# Data Cleaning retail_sales 
# step 1 : Handling missing values
# sales,profit,Order_date,city check 

select count(*) from retail_sales where Sales ="";
select count(*) from retail_sales where Profit ="" or Profit = "unknown" 
or Profit = "N/A";
select count(*) from retail_sales where Order_date = "N/A";
select count(*) from retail_sales where City ="";
...
update retail_sales set Sub_category = 'Accessories' where 
Sub_category = 'Accessoriess';
update retail_sales set Sub_category = 'Appliances' where 
Sub_category = 'Appliancess';
update retail_sales set Sub_category = 'Bookcases' where 
Sub_category = 'Bookcasess';
update retail_sales set Sub_category = 'Furnishings' where 
Sub_category = 'Furnishingss';
update retail_sales set Sub_category = 'Machines' where 
Sub_category = 'Machiness';
update retail_sales set Sub_category = 'Storage' where 
...
Segment,Country,City,State,Postal_code,Region,Product_id,
Category,Sub_category,Product_name,Sales,Quantity,Discount,Profit
order by Row_id) as rn from retail_sales) as t where rn>1;

set sql_safe_updates= 0;

delete t1 from retail_sales as t1 join(select Row_id ,row_number() 
over(partition by Order_id, 
Order_date,Ship_date,Ship_mode,Customer_id,Customer_name,
Segment,Country,City,State,Postal_code,Region,Product_id,
Category,Sub_category,Product_name,Sales,Quantity,Discount,Profit
order by Row_id) as rn from retail_sales) as t2 
...
update retail_sales set Discount = null where Discount< 0 or Discount >1;

# Negative Quantity: sign error, fix with ABS

update retail_sales set Quantity = abs(Quantity) where Quantity < 0;
set sql_safe_updates = 1;

# Verify 

select count(*) from retail_sales where Sales < 0;
select count(*) from retail_sales where Discount < 0 or Discount > 1;
...
# Total Profit Margin %
select (sum(Profit)/sum(Sales))*100 as Profit_Margin_Percent from retail_sales;

# Total Distinct Order count 
select count(distinct Order_id) as Total_Orders from retail_sales;
...
from retail_sales where Sales is not null ;
...
from retail_sales group by Segment;
